...
- **Click-through**: `setIgnoreCursorEvents` is async IPC. On macOS, **debounce re-enabling pass-through** (~180ms) so panel clicks are not stolen by Finder; arm panel headers on `pointermove` and call `setFocus()` when hovering a drag handle while unfocused. `suspend()`/`resume()` brackets any drag; `deferPassThrough()` after drag ends.
