...
| File | Owns | Read when |
|---|---|---|
| `index.md` (this file) | Documentation map | First. Always. |
...
| `PROJECT_FEATURES.md` | Shipped feature record (implementation detail for reconstruction) | After completing a task; before re-implementing existing behavior |
...
- Documentation lives here only — not duplicated at repo root (except slim `README.md`).
- Decision records: `docs/decisions/<NNN>-*.md` only.
- Shipped features: `docs/PROJECT_FEATURES.md`.
- After doc moves: run `skills/audit-project/scripts/audit.ps1`.
