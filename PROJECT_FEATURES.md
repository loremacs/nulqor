...
### 0.2 Repo layout
**Files:** `docs/`, `extensions/`, `skills/`, `TASKS.md`, `AGENTS.md`, `README.md`
...
**Key functions:** `Loader::scan_and_load(extensions_dir, root, ctx)`.
...
**Purpose:** Mounts window shell. Emits `canvas:ready@1` after all extensions activate. Proves loader + bus + IPC end-to-end.
...
### 2.1 Provider extension (`extensions/provider-lmstudio/`, `ext_provider_lmstudio.rs`)
...
### 2.2 Transcript extension (`extensions/transcript/`, `ext_transcript.rs`)
...
### 2.3 HTTP API extension (`extensions/http-api/`, `ext_http_api.rs`)
**Purpose:** axum HTTP server (port 8080) + WebSocket. Exact endpoint surface from decisions/006 §1–3.
**Endpoints:** `GET /health`, `POST /connect`, `GET /models`, `POST /message`, `POST /register_observer`, `GET /catch_up`, `POST /ack_observer`, `GET /list_observers`, `GET /transcript`, `GET /ws`.
...
### 2.4 Chat UI panel (`extensions/chat-panel/`, `src/main.ts`, `src/style.css`)
...
### 2.5 Context editor extension (`extensions/context-editor/`, `ext_context_editor.rs`)
...
### 2.6 MCP bridge extension (`extensions/mcp-bridge/`, `ext_mcp_bridge.rs`)
...
### 3.1 Skill-runner extension (`extensions/skill-runner/`, `ext_skill_runner.rs`)
...
### 3.2 Validation extension (`extensions/validation/`, `ext_validation.rs`)
...
### 3.4 Run-logger extension (`extensions/run-logger/`, `ext_run_logger.rs`)
